 ...
 */
 ...
 const organizedData= splitData.reduce(
     (prev, curr, i) => {
       const position = Math.floor(i / 2);
       prev[position] = [].concat(prev[position] || [], curr);
       return prev; 
     },
     []
   ).map(val=>{
     const key=val[0].split("=")[1]
     const age=val[1].split("=")[1]
     return {key,age}
 
   })
   const checkAge=organizedData.filter(data=>data.age==="32")
   console.log(checkAge.length, "items")
 
   let outputStream = fs.createWriteStream(path.join("npm-tests",'output.txt'));
